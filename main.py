...
def refresh_table(app, res_df) -> None:
    def update_outputs(event) -> None:
        ...
        with app.imageDiv:
            ...
            freq.on('blur', lambda: h.set_content(cm.draw(for_web=True, F=float(
                freq.value) if freq.value else None).decode('utf-8')))

    if res_df is not None:
        # We only want the numerical results data, not the objects column
        ...
        app.gridDiv.clear()
        with app.gridDiv:
            ui.aggrid.from_pandas(subset, options=table_options).on(
                'rowDoubleClicked', update_outputs, ['rowId']).style(style)
...
def fit(app) -> None:
    if app.zlist:
        ...
        result = ex.do_experiment(
            app.zlist, ex.all_components, depth, ct.cost_max_swr)
...
async def update_inputs(app) -> None:
    ...
    match app.distr.value:
        case 0:  # uniform
            app.zlist = ex.random_points_uniform(
                (rmin, rmax), (imin, imax), int(app.points.value), plot=False)
        case 1:  # gaussian
            sr = (rmax - rmin) / 6  # Approximate one standard deviation
            mr = (rmax + rmin) / 2  # Calculate the mean
            si = (imax - imin) / 6
            mi = (imax + imin) / 2
            app.zlist = ex.random_points_gaussian(
                mr+1j*mi, sr, si, app.corr.value, int(app.points.value), plot=False)
        case _:
            pass
    rc = [z/50 for z in app.zlist]  # Normalize to 50 ohms
    app.fig.update_traces(patch={"imag": np.imag(
        rc), "real": np.real(rc)}, selector={'name': 'inputs'})
    app.plot.update()
    ...
    def setup_ui(self):
        ...
        with ui.row():
            ...
            with ui.column().classes('gap-2'):  
                ...
                with ui.element('div').classes('p-2 space-y-2 self-center'):
                    with ui.row().classes('items-center'):
                        self.points = ui.input(value=INI_POINTS, label='Points').classes(
                            'w-32').props('square outlined dense')
                        self.distr = ui.select({0: 'Uniform', 1: 'Gaussian'}, value=0, label='Distribution',
                                               on_change=lambda x: test.set_visibility(
                                                   True) if x.value == 1 else test.set_visibility(False)
                                               ).classes('w-32').props('square outlined dense')
                        ui.button('Plot', on_click=lambda: update_inputs(
                            self)).classes('w-32')

                    # real value slider
                    with ui.row().classes('items-center'):
                        ui.label('Real:')
                        self.r_range = ui.range(min=MIN_R, max=MAX_R, value={
                                                'min': 10, 'max': 60}).classes('w-72')
                    ...
                    with ui.row().classes('items-center'):
                        ui.label('Imag:')
                        self.i_range = ui.range(min=MIN_I, max=MAX_I, value={
                                                'min': 5, 'max': 20}).classes('w-72')
                    ...
                    with ui.row().classes('items-center') as test:
                        ui.label('CorCoef:')
                        self.corr = ui.slider(
                            min=-1, max=1, value=0, step=0.1).props('selection-color="transparent"').classes('w-72')
                        ui.label().bind_text_from(
                            self.corr, 'value', backward=lambda v: f'{v}')
                        test.set_visibility(False)

                    # fit options
                    with ui.row().classes('items-center'):
                        ui.select(['Max SWR', 'Mean SWR'], value='Max SWR', label='Minimize').classes(
                            'w-32').props('square outlined dense')
                        self.levels = ui.input(value='2,3', label='Levels').classes(
                            'w-32').props('square outlined dense')
                        ui.button('Fit', on_click=lambda: fit(
                            self)).classes('w-32')
                ...
                with ui.element('div').classes('p-2'):
                    ...
                    self.fig.update_layout(
                        margin={"l": 25, "r": 25, "t": 25, "b": 25}, autosize=False, width=500, height=500)
                    self.fig.add_trace(go.Scattersmith(smith_dict))
                    self.fig.add_trace(go.Scattersmith(
                        {**smith_dict, 'marker_color': 'blue', 'name': 'outputs'}))
            ...
            with ui.column().classes('gap-2'):
                ...
                self.gridDiv = ui.element('div').style(
                    'width: 550px;').classes()

                # --- image placeholder ---
                ui.label('Schematic').tailwind(*section_style)
                self.imageDiv = ui.element('div').classes(
                    'w-full').style('position: relative;')
